...
//         return WError(
//           onRetry: onRetry,
//           message: errorMessage ?? controller.errorMsg(),
//         );
...
import 'package:momo/core/extensions/ex_expanded.dart';
import 'package:momo/core/widgets/load_and_error/models/view_state_model.dart';
import 'package:momo/core/widgets/load_and_error/widgets/error_section.dart';
import 'package:momo/core/widgets/load_and_error/widgets/loading_widget.dart';
import 'package:momo/core/widgets/load_and_error/widgets/no_data_found_section.dart';
  ...
  Widget build(BuildContext context) {
    ...
    switch (currentState) {
      case ViewState.initial:
        child =
            const SizedBox.shrink().withKey(const ValueKey("viewStateInitial"));
        break;

      case ViewState.loading:
        if (hasContent) {
          // ✅ Data already available → show shimmer on top of loaded widget
          child = _buildShimmer(loadedWidget)
              .withKey(const ValueKey("viewStateLoadingWithExistingData"));
        } else {
          // ⏳ No data yet → show loading placeholder or shimmer
          child = (loadingWidget ?? _buildShimmer(WOnScreenLoading()))
              .withKey(const ValueKey("viewStateLoadingNoData"));
        }
        break;
      ...
      case ViewState.loaded:
        if (hasContent) {
          ...
          child = (emptyWidget ?? WNoDataFound())
              .withKey(const ValueKey("viewStateLoadedEmpty"));
        }
        break;
      ...
      case ViewState.empty:
        child = Align(
          ...
          child: (emptyWidget ?? WNoDataFound())
              .withKey(const ValueKey("viewStateEmpty")),
        );
        break;
      ...
      default:
        child =
            const SizedBox.shrink().withKey(const ValueKey("viewStateNone"));
        break;
    }
